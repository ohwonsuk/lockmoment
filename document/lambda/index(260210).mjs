...
export const handler = async (event) => {
    ...
    try {
        ...
        if (httpMethod === 'POST' && path === '/auth/kakao') {
            const { kakaoAccessToken, role } = data;
            ...
            if (!kakaoResponse.ok) {
                return response(401, { success: false, message: '카카오 인증 실패' });
            }
            ...
            if (userResult.rows.length === 0) {
                ...
                // 신규 사용자 생성
                await client.query(
                    `INSERT INTO users (id, auth_provider, kakao_user_id, email, display_name, created_at, updated_at)
                     VALUES ($1, 'KAKAO', $2, $3, $4, NOW(), NOW())`,
                    [
                        userId,
                        kakaoUserId,
        ...
        if (httpMethod === 'POST' && path.includes('/presets/') && path.includes('/apply')) {
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`,
                [crypto.randomUUID(), presetId, user.userId, target_type || 'STUDENT', target_id, preset.purpose, policyId]
            );
          ...
          WHERE q.id = $1 AND q.status = 'ACTIVE'`,
            ...
            if (qrInfo.purpose === 'ATTENDANCE_ONLY' || qrInfo.purpose === 'LOCK_AND_ATTENDANCE') {
             ...
             ON CONFLICT (qr_id, student_id) DO UPDATE SET status = 'PRESENT', created_at = NOW()`,
                        [crypto.randomUUID(), qr_id, targetClassId, device?.user_id, device?.id]
                    );
                }
            }
            ...
            await client.query(
                `INSERT INTO qr_device_usage (id, qr_id, user_id, device_id, used_at)
                 VALUES ($1, $2, $3, $4, NOW())`,
                [crypto.randomUUID(), qr_id, device?.user_id || '00000000-0000-0000-0000-000000000000', device?.id]
            );

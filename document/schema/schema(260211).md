-- 앱 권한상태 추적 대응 Devices 및 다른 테이블 삭제 후 재생성(260210)
...
-- devices 테이블 생성 (260210)
CREATE TABLE devices (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  
  -- user_id: 실제 기기 사용자 (자녀/학생)
  ...
  -- 🔐 권한 상태 (핵심)
  accessibility_permission permission_state_enum, -- Android 접근성 / iOS 해당 없음
...
-- 기존 parent_child 테이블 삭제 (260210)
DROP TABLE IF EXISTS parent_child CASCADE;

-- 신규 parent_child_relaltion 테이블 생성 (260214)
CREATE TABLE parent_child_relations (
  ...
  nickname VARCHAR(50), -- 부모가 설정한 자녀의 이름(별명)
...
-- 자녀 스케쥴 테이블 생성 (260210)
CREATE TABLE child_schedules (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),

  child_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,

  name VARCHAR(100) NOT NULL, -- 예: "평일 학습 시간"
  description TEXT,
  ...
  created_by UUID REFERENCES users(id), -- 부모/교사
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
...
-- 서버 공통 앱 카테고리 정의 (260212)
CREATE TABLE app_categories (
  id VARCHAR(50) PRIMARY KEY,       -- 'EDUCATION', 'GAMES' 등 (Canonical ID)
  display_name VARCHAR(50) NOT NULL, -- UI 표출용 명칭 (예: '교육', '게임')
  ios_category VARCHAR(100),         -- iOS ActivityCategory (예: '.education')
  android_label VARCHAR(100),        -- Android 대응 라벨 (예: 'Education')
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
-- 앱 카테고리 매핑 테이블 (260212)
-- Android 패키지명 또는 iOS 번들ID를 공통 카테고리로 매핑
CREATE TABLE app_category_map (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  
  platform device_platform_enum NOT NULL,         -- 'ANDROID' 또는 'IOS'
  package_name VARCHAR(255) NOT NULL,            -- 패키지명(Android) 또는 번들이름(iOS)
  app_name VARCHAR(100) NOT NULL,                 -- UI 표출용 앱 명칭 (예: '카카오톡', 'Google Classroom')
  category VARCHAR(50) NOT NULL 
    REFERENCES app_categories(id),                -- CANONICAL 카테고리 참조
...
-- User Role 테이블 생성 (260211 수정)
-- scope_id가 NULL이면 GLOBAL 권한으로 간주합니다.
...
CREATE TABLE user_roles (
  ...
  -- 🌐 권한 범위
  -- GLOBAL: 전체 유효 (SERVICE_ADMIN, TESTER 등)
  -- ORG: 특정 기관 한정 (ORG_ADMIN, TEACHER 등)
  -- CHILD: 특정 자녀 한정 (PARENT 관리용)
  scope_type TEXT CHECK (scope_type IN ('GLOBAL', 'ORG', 'CHILD', 'CLASS')) DEFAULT 'GLOBAL',
  scope_id UUID, -- NULL이면 GLOBAL, 그외 organization_id, child_user_id 등

  assigned_at TIMESTAMPTZ DEFAULT now(),

  -- 동일 사용자가 동일 범위 내에서 동일 역할을 중복해서 가지지 않도록 보장
...
-- QR 코드 테이블 (260214) hmac_sig, updated_at 추가
...
-- 📌 사전 등록(Preset) 기본 데이터 추가 (UI 테스트 및 초기 사용용)
...
VALUES 
  (gen_random_uuid(), 'SYSTEM', '수업 집중', '학습 앱만 허용하고 나머지는 차단합니다.', 'LOCK_ONLY', 'APP_ONLY', '["EDUCATION", "UTILITIES"]', '["GAMES", "SOCIAL", "ENTERTAINMENT"]', '[]', 60),
  (gen_random_uuid(), 'SYSTEM', '시험 모드', '모든 앱을 차단하여 시험에만 집중합니다.', 'LOCK_ONLY', 'FULL', '[]', '[]', '[]', 120),
  (gen_random_uuid(), 'SYSTEM', '취침 시간', '밤 늦은 시간 스마트폰 사용을 제한합니다.', 'LOCK_ONLY', 'APP_ONLY', '["HEALTHVITALITY"]', '["GAMES", "SOCIAL", "ENTERTAINMENT"]', '[]', 480);

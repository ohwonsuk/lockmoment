...
-- 다중 보호자(Many-to-Many) 및 다중 기관 소속 지원 통합 스키마

-- 1. 기초 분류 및 Enum (기존 통합)
...
CREATE TABLE users (
  ...
  -- 🔐 보안 및 제어 (260219 추가)
  pin_code VARCHAR(6),                -- 내 정보 접근용 6자리 PIN
  restrict_my_info BOOLEAN DEFAULT FALSE, -- 부모의 자녀 화면 접근 제한 플래그
...
-- 3. 관계(Relation) 테이블 고도화

-- [부모-자녀] 다중 보호자 지원 (Many-to-Many)
CREATE TABLE parent_child_relations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  parent_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  child_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  nickname VARCHAR(50),               -- 부모가 설정한 자녀의 별명
  is_primary BOOLEAN DEFAULT FALSE,   -- 주 보호자 여부 (최대 2명 권장이나 구조상 N명 가능)
  created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE active_locks (
  ...
  initiated_by UUID REFERENCES users(id), -- 잠금을 시작한 주체
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
